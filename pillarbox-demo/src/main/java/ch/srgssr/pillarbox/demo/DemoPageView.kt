...
fun SRGAnalytics.trackPagView(demoPageView: DemoPageView) {
    val comScorePageView = ComScorePageView(title = demoPageView.title)
    val commandersActPageView = CommandersActPageView(title = demoPageView.title, type = "DemoScreen", levels = demoPageView.levels)
